...
- **Semantic contract:** 虽然事件名叫 `permission.required`，它既可承载 ask，也可承载同一 `request_id` 的 resolved 更新；consumer 必须用 `decision` 区分 `ask / allow / deny`，而不是靠事件名分流。
- **Ask-state scope:** 当 `decision: "ask"` 时如果出现 `scope`，它只是推荐/预选授权范围提示，不代表权限已经授予。
...
- **Decision endpoint:** 用户在 `PermissionCard` 上的提交必须发送到 `POST /sessions/{session_id}/permissions/{request_id}/decision`
