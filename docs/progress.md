...
- IAM 唯一 writer 已完成 E2 文档门：三设计文档/CURRENT/ADR-005 明确已固定 Agent/Platform artifacts、专用 Platform tenant-machine 持久 purpose + 独占 scope、当前 client 重读，以及响应 `caller_client_id` 的 rotation barrier；Platform ingress 留后续 owner 切片。当前仅文档未提交，endpoint/合同生成/真实事务仍待实现与 Root 复验。
