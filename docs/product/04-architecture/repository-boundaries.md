...
当前主干优先级先收拢这三个运行时仓：`kokoro-web`、`kokoro-session`、`kokoro-agent`。其他未来中台仓不是这轮协议治理的重点。
...
- 任何协议字段、事件名、边界类型改动，都必须同步检查三处：spec、`kokoro-session` producer、`kokoro-web` consumer。
- 如果改动来自 agent 原始事件，还必须同步 `kokoro-agent` emitter 与 `kokoro-session` agent-event parser。
- 不允许让三个主仓各自维护语义漂移的重复边界类型；契约 owner 改了，spec 和上下游实现也必须同一轮收敛。
