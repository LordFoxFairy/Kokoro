...
|---|---|---|---|
| kokoro | LordFoxFairy/kokoro-app | Web 产品、同源 /api/*、页面状态/SSE | aa59e35 |
| kokoro-bff | LordFoxFairy/kokoro-bff | Chat、业务 BFF、Project/Task/ScheduledTask、适配/幂等 | 406dfc4 |
| kokoro-agent | LordFoxFairy/kokoro-agent | Run、执行、HITL、恢复、事件投影、HTTP ingress | a82e4ef |
...
kokoro-agent 指向 a82e4ef；其余 9 个目录是同目录独立 checkout，不是 Root 的业务子目录。
