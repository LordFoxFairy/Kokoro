...
- Web `5299f29…` 已实现同源 issuer GET、sign-in 与 tenant/consent 三页交互，并安装 Auth.js RP-only Code+S256/state/nonce callback。固定 issuer/client/callback/resource，token Basic、userinfo Bearer 与 JWKS 仅经 BFF；验证型 `client.callback` 检查 EdDSA ID token，Redis 一次消费 state。Next 反代内部 authority 与公开 origin 不同的误拒已按固定 Host/Origin 修复；本次又将 IAM 真实 consent 回调的唯一 `code/state/iss` 与固定 issuer 同时纳入 relay 和 RP 严格校验。Node22 contract 52/52、architecture 32/32、标准 1345/1345、build 与 Playwright 6/6；真实 Next + 严格 BFF fixture 覆盖签名、重放、取消、超时和限额，HTTP 代理式测试覆盖公开 Host 与内部 URL 不同。验证成功仍受控 `503 product_session_unavailable`、无可用新 session；fixture 不等于真实 IAM 三服务或完整登录。
