...
| --- | --- |
| `apps/kokoro-app` | `14e23e602a5631009584d84f58871e51d32b821c` |
...
- Web `14e23e6…` 已实现同源 issuer GET、sign-in 与 tenant/consent 三页交互。`/auth/select-tenant|consent` 只将精确 GET 导向静态 `/iam/interactions/*`，以接收 IAM `Path=/iam` session cookie；内层以一次性 Redis CSRF、owner 列表重核和 IAM 最终签名验证续接。HEAD/OPTIONS 与直接 owner mutation 均拒绝。Root Node22 contract 52/52、architecture 32/32、标准 1320/1320、build 与 Playwright 6/6；真实 Next + Path-aware CookieJar 使用严格 BFF HTTP fixture，不等于真实 IAM/RP 登录。
...
1. Web Auth.js RP：Code+S256/state/nonce、固定 callback、server-only token/userinfo、唯一 resource；替换目前 consent 成功时受控 `503 rp_callback_unavailable`。实现前核对当前 IAM client readback 与 Auth.js/Next/Node 兼容。
...
3. Refresh CAS 与先 tombstone 后 revoke/end-session 的退出链，再做真实 Web→BFF→IAM 浏览器组合及越权/失效/重放负例。之后推进单一 AG-UI 聊天、Agent/Storage/System/Platform/Scheduler 各自 owner 闭环；Billing 最后。
