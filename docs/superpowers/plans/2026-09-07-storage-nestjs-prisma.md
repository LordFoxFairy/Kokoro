...
配置预裁决：DB从亲建handle URL仅按显式网络authority映射给container，保留随机库名/凭据/query，禁止第二任意DB URL；实际RPC receipt/完整Prisma关系由父侧同handle证明同库。AWS资格必须明确host与container可消费的同源凭据环境，不自动挂载home/profile、查找导出凭据；Docker --env NAME只在argv给名字，child env持值，diagnostic不转储inspect Env。静态custom不暗丢session token。镜像tag先解析immutable imageID，run使用该ID并核对container.Image/CMD/entrypoint/label；release的OCI archive来源仍需后继绑定，imageID不是registry index digest的替代。
