...
**Repository/writer:** Root / Root integration subagent. **Reviewer:** cross-repository reviewer. Requires Task 5 Capability/BFF real smoke accepted. Later listener-release and Scheduler-smoke prerequisites belong to Task 11, not this already completed initial Capability activation; unchanged historical artifact provenance remains intact.
...
**Owner/writer:** Root smoke subagent, dispatched after Task10 source freezes and its code review passes. Task10 real-CLI environment readiness is not a data dependency of this independent repair: if it still awaits loopback DNS, serialize Task5R after the stopped Task10 writer without declaring Task10 accepted; Task11 still requires both real smokes green. **Exact files:** `scripts/e2e/run_capability_bff_smoke.py`, `scripts/e2e/capability_bff_smoke_runtime.py`, `scripts/tests/test_capability_bff_smoke.py`, `scripts/INDEX.md`. Preserve tests/fixtures already accepted; no child, contract, inventory or gitlink change. Port only the reviewed resource ownership/acknowledgement-loss behavior needed by this runner, with RED/GREEN for uncertain CREATE/SET, before-create failure, pre-existing resource preservation and cleanup-query errors. Root also independently reproduced its readiness fixture retaining one handler after a partial-header client and context exit; apply the same bounded owned-HTTP lifecycle fix and assert no leftover handler, without shared-infrastructure access. No new cross-runner common framework. Advance BFF runtime input to accepted `5ea4440941ed65c424fffb0ae834e67b2ae93e74`, preserving precise wrong-SHA rejection, so real validation uses the available pushed child. Root reruns tests and the exact eight-case CLI before an independent four-file commit. Task11 then integrates the new composition and does not duplicate this completed pin repair.

**Task5R placement gate (Root, 2026-09-22):**

| Item | Decision |
| --- | --- |
| Owner/current state | Root smoke only; current runner884/test408 lines, focused baseline26 passed. Capability7f89a267 is clean; BFF5ea444094 is the already accepted input. Existing tests model only failed-before-CREATE, so they do not cover a successful create with lost acknowledgement. |
| Location alternatives/granularity | Add `capability_bff_smoke_runtime.py` alongside the existing runner for resource/process/readiness lifetime; reject growing the already884-line mixed runner or importing Scheduler-specific fixtures into Capability. Keep case assertions and CLI in the runner. Move existing definitions rather than re-export aliases; tests import their actual owner. No common framework or new directory. |
| Dependencies/data/API | Runner imports its runtime; runtime never imports runner or child code. Own random `w0b_cap_<run>_{capability,bff}` databases and exact capability harness prefix, preserving pre-existing resources. No child/schema/API/owner identity change, no Scheduler-native-key rules copied into Capability. Use existing PG/Redis; all HTTP fixtures remain127.0.0.1. |
...
Task5R temporary existing-test exception: its unchanged midflight cleanup test is105 lines. Root owns a narrow exception until2026-09-23 or before Task5R real-runtime acceptance, whichever first; extract cohesive fixture setup during the pending owner-pin/loopback update without dropping assertions. Runner/runtime remain within800/100 now; do not claim all tests already meet the function limit.
...
**Commit/evidence separation for an environment-blocked activation:** after all changed child releases pass their own gates and are pushed, Root may accept their gitlinks and refresh every affected commit-blob digest, truthful broken reason, and smoke release input without activating additional edges. This keeps the reproducible code composition current and avoids blocking independent code cleanup on the local Scheduler callback DNS condition. In that partial integration, require the existing `w0b-capability` checkpoint (exact2 active/14 broken/1 illegal), Root tests and clean topology; Task11/15 remain awaiting real smoke/activation, and `w0b-exit` is not claimed. Edge activation still requires both final real smokes and the exact4/12/1 exit checkpoint. No provider/listener/DNS boundary is weakened.
...
The exact combined scope is the existing13-file union below plus canonical `contract/openapi/v1/openapi.yaml`, `.env.local.example`, `.env.prod.example`, `INDEX.md`, `docs/ACCEPTANCE.md`, `test/doubles/mock-route.ts`, `test/doubles/bff-store.ts`, `src/contracts/account.ts`, and `contract/README.md` (22 existing files, no new directory). The actual canonical operation currently lacks503; add ErrorEnvelope with the exact `storage_integration_unavailable` code constraint. User-approved prelaunch clean-slate also removes the unreachable Library200 response and orphan LibraryResponse/LibraryItem schemas/type, rather than keeping a speculative future-success contract. Record this explicitly as a user-approved prelaunch corrective baseline in contract/README; do not present it as backward compatible or weaken the postlaunch breaking policy. Preserve the operation identity,403 admission and existing trace envelope; no global error-system or IAM rewrite. Live and explicit test composition both fail closed. Keep route recognition needed to reach the503 handler, but delete old outbound transport,parser,env and fake data. Negative regression literals in tests are intentional; do not weaken assertions to make a blanket grep pass. Storage edges remain broken until W1/W2 succeeds.
