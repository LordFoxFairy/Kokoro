...
4. Public list reads support only exact category/source/state filters and stable keyset pagination over active explicit entries. The first page returns an opaque `snapshotRef + spaceVersion`; every continuation cursor binds scope, normalized filters, ordering and that exact pair. Any mutation that changes public entry content/head, priority, active membership, forget or reset state advances `spaceVersion`; an old continuation fails stale. Succeeded commands replay their exact committed version and detail reads expose the observed version. Content FTS/trigram and relevance ranking remain M1a; M0.1 creates no search index or `MemorySelectionSnapshot`.
...
6. Export freezes a cutoff and produces a versioned encrypted artifact; status and delivery use opaque refs and current authorization.
