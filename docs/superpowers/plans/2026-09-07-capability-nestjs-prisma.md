...
由此选定公开 Prisma API 的单事务 installer 路径，不引入SQL splitter、手写seed、adapter私有连接、两阶段补行或runtime self-heal。现续派原 capability_owner_p1b 仅修改 child 既有：`docs/ADR/ADR-001-nestjs-prisma.md`、`docs/TECHNICAL_DESIGN.md`、`docs/API_CONTRACT.md`、`docs/DATA_MODEL.md`、`docs/RELIABILITY.md`、`docs/RUNBOOK.md`、`docs/CURRENT.md`；不得改 schema/generated/src/scripts/test/package/env/proto/OpenAPI 或 Git。必须收敛：
...
6. 全部七文档把不存在的`grant_id`改为wire `invocation_grant`（概念类型MCP invocation decision）；保留`mcp-grant:<uuid-v4>`为值格式描述，不新增SQL/wire字段或alias。继续固定Skill `series_id/skill_id/source_ref=skill:<skill_id>/installation_id`、MCP `connector_id/server_id/connection_id/authorization_id/invocation_grant`、command/event/recovery/retirement/delivery typed identity不互换及Manus list/discover→opaque typed reference原则。
