...
#### AGENT-EXECUTION-PROOF-A2 计划验收与 A2a 实现授权（2026-09-12）
...
Fix R1 仍由原 writer 单一写入，只授权继续修改两个 production 文件与两个 focused tests；其他6个已修改文件保持冻结，只有修正事实表述确有必要时先报告 Root。实现必须：signer入口拒绝非exact `ExecutionProofInput`/config，紧邻签名前从exact字段重建并重验immutable snapshot；identity逐字段验证exact `kind in {user,project,service}`、built-in非空string并投影为exact `{kind,opaque_ref}`，禁止多态`model_dump`；所有受信string使用exact built-in type。config在构造时复制已验证的issuer/kid/key快照且issue前仍验证其不变量。compact结果只接受exact built-in `str`，立即转plain ASCII bytes、全程按bytes分段/校验，最终由验证后的bytes解码返回。

R1测试先分别对上述缺口取得RED，再保持A1 exact vector GREEN；覆盖duck-typed/frozen-mutation input、非法/子类identity、duck config均在`jwt.encode`前失败；恶意`str`子类失败。三段各覆盖padding和same-decoded-bytes trailing-bit alias；显式拒绝Ed448；AST helper解析`import jwt as`、`from jwt import encode/api_jws`及signer import alias，并用合成源码证明门本身能抓绕过且不误报无关同名方法。固定可达的exact 16384-byte compact接受及下一可达超限拒绝。仍禁止A2b/A2c、A1 artifact/checker、其他文档/dependency/HTTP/worker/DB/Redis/IAM/Platform/Capability与Git/index；修复后重新跑完整门、冻结并由同一SPEC/QUALITY复审。

Fix R1 冻结为同一 Agent HEAD、10 files / 86335 bytes、dirty SHA `0e4d6fb317616e21441ba6a8e9819bab6dba1a8277eb9ef4a08b3b5ebe9e5646`；首轮两个Blocking和三段alias/Ed448均闭环，focused 74、contract 182、full default 742 passed/6 skipped/77 deselected及静态/build门通过。但SPEC为`0/1/0`、QUALITY为`0/2/1`，仍未放行：AST helper漏报`from . import execution_proof_signer`及更深relative imports；`raise ... from None`仍让外层异常的`__context__`保留含secret的dependency异常；所谓next reachable oversize为16386，实际16385可由exact fixture追加一个普通字符构造。

Fix R2 仍由原 writer 单一写入，只授权修改 `src/kokoro_agent/execution/execution_proof_signer.py`、`tests/contract/test_execution_proof_runtime.py`、`tests/unit/execution/test_execution_proof_signer.py`，其余7个文件逐字节冻结。先取RED：JOSE/JCS/crypto路径抛secret sentinel时暴露异常的cause/context递归均不得含sentinel，推荐均为None；在`except`内只标记失败并在退出except后创建/抛出脱敏异常，不保存或记录原异常。AST gate按待查文件相对`src/kokoro_agent`的真实module path解析任意`ImportFrom.level/module/names`，覆盖`from . import execution_proof_signer [as ...]`、`from ..execution import ...`以及多层direct module import，同时保留JWT alias检测和无关同名方法不误报。16KiB测试固定exact 16384接受与真实16385拒绝。不得扩大范围；完成后重新冻结、完整验证并由同一SPEC/QUALITY复审。
