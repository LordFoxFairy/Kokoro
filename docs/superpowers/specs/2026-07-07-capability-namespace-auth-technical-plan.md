...
```text
docs/superpowers/specs/2026-07-07-capability-namespace-auth-technical-plan.md
```

它的作用是让人类先评审主线、边界、时序和工作包。它不是 handbook 权威章节，也不是 worker 派工单。

关系如下：

```text
docs/superpowers/specs/   当前方案怎么设计、怎么取舍
docs/kokoro-handbook/     拍板后的长期规则和稳定结论
docs/handoffs/            短期派工，解释这一轮怎么拆 worker
kokoro-*/docs/            子仓自己的实现细节、验收和运行说明
tmp/                      外部参考、截图、探索草稿和一次性中间产物
```

当这份方案被确认后，只把长期有效的结论提炼进 `docs/kokoro-handbook/`；不要把整篇讨论稿复制过去。
