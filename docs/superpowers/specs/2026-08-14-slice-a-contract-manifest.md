...
| `AgentRuntimeService.AckProjection` | `1:request_id`, `2:agent_run_id`, `3:consumer_key`, `4:epoch:uint64`, `5:projected_seq:uint64`, `6:rejected_seq:uint64 optional`, `7:rejection_code:string optional` | `1:stored_epoch:uint64`, `2:stored_projected_seq:uint64`, `3:stored_rejected_seq:uint64 optional`, `4:stored_rejection_code:string optional` |
...
`AckProjection` has two presence-aware shapes. A positive ACK leaves request fields 6–7 absent and response fields 3–4 absent. A first NACK supplies both `rejected_seq` and `rejection_code`: the rejected sequence is positive and exactly `projected_seq + 1`, while the code is nonblank UTF-8 bounded to 128 bytes. `projected_seq` remains the last successfully persisted Chat watermark, and `epoch` belongs to that positive watermark; when `projected_seq == 0` and no positive event exists, `epoch` is instead the rejected event's stored epoch for fencing. The server binds the rejected event by run-global `(agent_run_id, rejected_seq)` and validates its stored epoch. A successful first or exact-replay NACK echoes both immutable stored rejection fields exactly. Different rejection evidence conflicts.

Compatibility fails safe. An old Agent ignores request fields 6–7 and therefore sees only the unchanged last-good positive watermark; it cannot acknowledge the poison event. Its old response omits fields 3–4, so new Chat keeps quarantine and the exact retry record unless both echoes are present and equal the request. Partial presence, zero/non-next sequence, blank/oversize code map to `INVALID_ARGUMENT`; missing runs to `NOT_FOUND`; unpublished evidence to retryable `FAILED_PRECONDITION`; exhausted dependency retries to `UNAVAILABLE`; future/stale fences to retryable `ABORTED`; first-rejection drift to `ALREADY_EXISTS`; unexpected failures to redacted `INTERNAL`. Every error echoes the validated nonblank `request_id`, keeps the existing 512-byte diagnostic bound, and a stale-fence error includes `current_generation`.

Only the existing Agent, Chat and Root E2E consumers receive this private runtime contract. Root E2E adds `agent_runtime.proto` and its transitive `agent_events.proto` because backend NACK compatibility is exercised end to end; no Site runtime, caller or consumer is added. Generated provenance is a deliberate second step: commit this source authority first, then generate Root E2E and the Agent/Chat descendants from that exact clean source SHA.
