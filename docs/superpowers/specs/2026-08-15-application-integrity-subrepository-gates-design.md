...
| Root 集中保存全部服务 Proto、生成产物和测试 | 服务提供方拥有自己的 RPC Proto/生成配置；业务测试归服务仓，跨仓测试归独立 system-tests 仓 |
...
1. Site、IAM、Model、Capability、Agent、Chat、Storage、Entitlement/Credit、Payment 和 Web 分别存放在独立 Git repository；各仓自己拥有 source、migration、RPC provider contract、生成配置、业务/管理接口、测试、报告、CI 和 dev entry。
...
4. `Kokoro` Root 退化为 meta repository，只管理文档、独立仓 pin/release manifest 和最终原子提升，不保存任何服务的业务 DDL、ORM schema、业务实现、生成 RPC consumer 或仓内测试。
5. `kokoro-protocol` 是独立薄仓，只拥有 `CommandIdentity`、`PrincipalContext`、`ErrorDetail` 等跨服务 wire primitives 和 Buf 基线，不收纳任何服务业务 RPC。
6. `kokoro-quality` 是独立薄仓，只拥有测试报告 JSON schema、证据 validator 和发布包，不收纳任何服务测试清单或测试实现。各服务仓固定版本调用它，但自己的 catalog、evidence、report 和 CI 仍保存在本仓。
7. `kokoro-system-tests` 是独立 Git 仓，只拥有公开 RPC pair、真实浏览器系统链、进程编排和跨仓报告聚合；不得复制仓内 unit/integration/admin 测试。
...
只有两侧 owner 报告均 `APPROVE`、provider Proto 已冻结且 consumer 已从固定 descriptor 生成 client，`kokoro-system-tests` 才创建 pair cut。每对使用 generated client、真实进程、双方独立数据库和正式独立报告，覆盖正向、拒绝、重放、并发、deleted parent、authn/authz、timeout、lost response 与 restart；禁止 mock、direct handler、共享 SQL join 或 admin SQL 代替。
...
所有所需 pair `APPROVE` 后，由 `kokoro-system-tests` 使用各仓公开 dev entry 启动真实 Site/IAM/Model/Capability/Agent/Chat/Storage/Entitlement/Credit/Payment/Web user/admin，走 production adapters、generated clients、各仓独立数据库和真实 Chromium。冻结任务当前为：
...
最终 Definition of Done 是：每个独立 Git owner 仓的业务面、管理面、数据库/repository/readiness/integrity audit、测试、报告和 CI 独立 `APPROVE`；`kokoro-system-tests` 中每个 generated RPC pair 独立 `APPROVE`；两轮 fresh 真实浏览器用户链与后台管理链最终独立 `APPROVE`；Root 最后只提升已验收的独立仓 pins 和 release manifest。在此之前整体状态保持未晋级。
