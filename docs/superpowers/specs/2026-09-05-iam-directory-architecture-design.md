...
状态：**过程设计已采纳，R1 目录已审查集成日常主目录。** 已验源码 e938b91，当前主目录文档收口 1d78be6。
长期技术方案由 [IAM TECHNICAL_DESIGN](../../../kokoro-iam/docs/TECHNICAL_DESIGN.md) 承接，当前事实与验证见
[IAM CURRENT](../../../kokoro-iam/docs/CURRENT.md)；本稿保留方案比较和旧路径处置依据，不成为第二份长期实现手册。
以下第 1–2/9 节的旧主目录与旧候选描述属于恢复实施时的基线；目录完成不表示 SQL/API/运行治理全部验收。
