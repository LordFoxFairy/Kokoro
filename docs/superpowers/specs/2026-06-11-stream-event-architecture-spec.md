...
| **畸形 fallback** | 畸形 Last-Event-ID → 全量;单条脏 run.request → 跳过不杀调度循环;relay 单条脏事件 → skip-and-continue(终态仍落 replay);agent 模型解析失败 → 终态 run.failed(worker 存活);web 单条解析失败 → skip-and-continue |
...
| **✅ P2** 已完成 | 删域 cursor(envelope.cursor + web parseCursorSeq + normalize cursorSeq),seq 成为唯一领域排序源,web seq R2→R1。前置实证满足(db0 零 replay);session 66 + web 175 + verify envelope 6 字段 + 真实 redis SSE gate 零回归。三义 cursor 消掉一个 | — |
| **✅ P3** 已完成 | message_ref/message_id/messageId → 单一 `segment_id`(agent 分配 `{run_id}:seg_NNNN` 全局唯一),session 删 messageIdFor 重映射。三仓 + contract;agent 80 + session 66 + web 175 + verify + 真实 redis SSE gate(envelope payload 有 segment_id 无 message_id) | — |
...
| 债 | 内容 | 性质 |
|---|---|---|
| **✅ D1** (=step10) | run_agent 两个字节级相同的 `ref_for_segment_*` + 3 nonlocal → 折叠成 `_Segmenter`(current/complete)。pytest 74 segment-oracle + pyright 0 + 真实 redis SSE gate 全绿,行为零改 | 已完成 |
| **✅ D2** | normalize.ts `envelope` 泛型化:payload `AguiPayload<E>`(从判别联合 schema 推导),mapEvent 各分支恢复 per-kind 编译期字段保护。无 cast,行为零改,注入证明非 vacuous(打错字段 tsc 报错)。session 66 + tsc 0 | 已完成 |
| **✅ D3** | web state-schema `z.union` → `z.discriminatedUnion("kind")`(直接路由,更快+更好错误)。行为等价(disk 兼容),新增 state-schema oracle 4 测试,tsc 0 + web 177 | 已完成 |
| **✅ D4** | subagent_registry env JSON 手工 TypeGuard → pydantic strict 模型(strict + extra=forbid + strip)。多余键/非 str 现在拒收,domain dataclass 不变。TDD 6 边界 + pytest 80 + pyright 0 | 已完成 |
| **✅ D5** | use-conversation.ts(557→469<500):提取 composer-input + mode-presentation 两个独立纯函数职责;store/重连/回复作为会话生命周期内聚核心保留(强拆只增状态传递开销,ROI 负)。行为零改,web 177 + tsc 0 | 已完成 |
