...
| W1C-1 / P0 | BFF受控`/iam`原生协议relay / `w1c_bff_relay_owner`实现、来源重pin由Root集成 | BFF `1fae01e3…`、IAM `c9a27721…`、Web文档 `38683a5…`已推送main；Root gitlink/库存待提升 | 精确path/method服务例外、原生HTTP/cookie/redirect保持；BFF Node22标准248/248、contract25/25；此前7/8/11/System实测绑定旧804a pin，新pin须重跑，完整Code+PKCE/交互/令牌/logout仍待W1C-1H；状态：待集成验证 |
...
| W1C-1G / P0 | Root从gitlink commit blob核验BFF relay↔IAM发布证据 / `w1c_root_relay_verifier`实现、独立审查，Root集成 | Root脚本 `070b0589…`、旧两gitlink/库存发布 `4d2c149b…`；新IAM/BFF pin待Root提升 | 固定IAM allowlist/snapshot与BFF policy的commit/digest、route/method子集/disabled负例；旧pin真实CLI PASS、14/14聚焦，最新pin必须提交后复跑；状态：待新pin集成验证（来源门，不替代OAuth正向行为） |
| W1C-1H / P0 | Root真实IAM→BFF relay首次OAuth正向组合 / `w1c_oidc_smoke_writer`唯一Root脚本writer；Root审查 | IAM `c9a27721…` host与BFF `1fae01e3…` policy均已发布，新Root gitlink尚待提升；两个Root脚本写入中 | 真实HTTP经过BFF `/iam`证明首次未预同意Code+S256、签名交互续接、token/userinfo/logout与受控cookie/header，隔离资源清理；不修改owner源码、IAM数据库事实或Web页面；状态：进行中 |
| W1C-1I / P0 | IAM test-owned OIDC host精确public JWKS回环 / `w1c_iam_flow_owner`唯一IAM writer；Root审查/Git | IAM `c9a27721…` clean；Root真HTTP已到Code+S256/token/userinfo/revoke 200，带hint end-session 401 invalid_token；public HTTPS issuer在本地fixture没有Web同源回环 | 仅IAM `test/fixtures/web-oidc-flow-host.ts`和聚焦host integration test：对精确自身issuer `/jwks`的内部fetch映射到已启动fixture loopback，结束/失败恢复原fetch；不改生产认证/契约、任意外网fetch或Root runner。Root重跑owner聚焦/清理再发布IAM→BFF来源pin→Root；状态：待派工 |
| W1C-1J / P1 | BFF relay原生浏览器metadata审查 / BFF唯一writer待派；Root裁决 | Root真HTTP证明无hint end-session在BFF Node fetch代理下为400 invalid_request，当前`sec-fetch-mode:cors`非browser navigation；带hint的JWKS缺口先由1I修 | owner评审是否需精确保留可信浏览器navigation语义，不能透传可伪造安全头或扩开放行；给出真实GET/POST负例和最小代码切片；状态：待只读审查，不阻断有hint 1H |
| W1C-2 / P0 | Web Auth.js RP、同源adapter与Product Session / `w1c_web_design_owner`三文档writer；源码唯一writer待派 | Web三文档 `62da3f85…`与最终来源pin `38683a5…`已推送main；BFF `1fae01e3…` policy digest固定，Root gitlink/库存待发布 | 三文档独立复审0 P1/P2，源码/依赖未改；W1C-2A只读范围盘点已完成，下一步同源GET窄relay→Auth.js Code+S256/CSRF/Product Session/全代理唯一Bearer/旧IAM直连删除；状态：设计门已验收，源码待派 |
| W1C-2A / P0 | Web固定policy来源与同源只读`/iam`入口 / Web唯一源码writer，Root审查与Git负责人 | Web `38683a5…` main clean；BFF `1fae01e3…` policy v1 digest `cbb62a48…`、IAM `c9a27721…`；先于Web Auth.js与mutation | 仅固定snapshot/provenance、精确安全GET路由、原生status/header/多Set-Cookie、cookie过滤、限额/取消及负例；所有browser POST、end-session、userinfo Bearer和未知路由fail closed，不修改旧Product代理/会话或IAM/BFF。Root在Web唯一writer停写后重跑Web门并提交；状态：待派工 |
