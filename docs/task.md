...
| W1C-2 / P0 | Web Auth.js RP、同源adapter与Product Session / Web单仓分片唯一writer；Root审查/Git | Web三文档、2A、2B-1、2B-2及2C RP-only源码已分别发布，最新Web `c71aa3f5130ae52c7f76356ac38bdabaf551d4e9`；BFF `a4dbc333…`/IAM `6bc9b19…`来源固定 | 三页同源交互/CSRF和 Auth.js Code+S256验证已落地；Product Session、全代理Bearer、旧IAM直连删除及真实三服务组合未完成。状态：部分完成，不是登录闭环 |
...
| W1C-2C-Proxy / P0 | Web 固定公开来源与 Next 反代误拒修复 / 原writer容量错误后`w1c_proxy_web_owner_sol`接手，Root审查/Git | Web `c71aa3f…`→`12e07eb5874b085be989364606d485dce0722249`，13文件已推送main且clean；只改Web IAM/RP同源入口、共用Host/Origin准入、文档与真实Next代理式测试 | 五处内部URL origin误用清除；固定Host、POST精确Origin、GET可选Origin，路径/query/CSRF/跳转不退化，不信X-Forwarded；独立复审0 P0/P1/P2，Root Node22 contract52/architecture32/标准1344、lint/typecheck/build、Playwright6均PASS。HTTP代理式fixture非真实TLS，后者归2D。状态：Web owner切片已验收，Root gitlink/库存待当前组合提交 |
| W1C-2D / P0 | Root真实 HTTPS Web→BFF→IAM RP-only 组合 / 原writer容量错误后`w1c_root_smoke_fix_sol`接手唯一Root脚本writer；Root审查/Git | Root main `06b6c21413fdfe85d94cf51f5bb90bfeac97b626` 起点；Web现`12e07eb…`、BFF `a4dbc33…`、IAM `6bc9b19…`；仅两新增Root脚本，Root独占docs/index/commit，不改owner生产代码/schema/contract | 目标：浏览器同源 CSRF→signin→tenant→consent→callback，真实IAM EdDSA/JWKS/token/userinfo、最终503无新session；重放、Origin/CSRF负例与自有资源清理。Root候选已针对审查1P1/多P2定点修复，聚焦10/10、全脚本580通过，真实链未运行；Web反代修复已发布，Root先精确pin其gitlink/库存，随后再跑真HTTPS，不以静态绿代签。状态：进行中/待Root来源pin与实链 |
