...
| W1C-1 / P0 | BFF受控`/iam`原生协议relay / `w1c_bff_relay_owner`实现、来源重pin由Root集成 | BFF `cd1c2600…`、IAM `b838853a…`及Root来源`83022b65…`已推送main | 精确path/method服务例外、原生HTTP/cookie/redirect保持；BFF Node22标准248/248、contract25/25；当前pin真IAM7/Capability8/Scheduler11/System均复验，自有资源0。Root修后真OAuth 15-case与独立审查已通过，1H脚本待提交；状态：relay基础已验收、OAuth证据待发布 |
...
| W1C-1H / P0 | Root真实IAM→BFF relay首次OAuth正向组合 / 原脚本writer与首修writer、`w1c_oidc_runner_fix2`唯一Root脚本writer；Root与独立审查 | IAM `b838853…`、BFF `cd1c260…`及Root `83022b65…`已发布；两Root脚本待精确提交 | 第二轮四项P1/P2均修复、独立复审0 P1/P2；Root独立真HTTP首次无session登录→签名continue→tenant→consent→Code+S256/token/userinfo/revoke/logout **15/15**，Root全scripts **563 passed、56 subtests**，自有资源0。无hint HTML logout只单测，归1J；状态：代码待提交后发布审计 |
...
| W1C-2 / P0 | Web Auth.js RP、同源adapter与Product Session / `w1c_web_design_owner`三文档writer；源码唯一writer分片推进 | Web三文档 `62da3f85…`与最终来源pin `38683a5…`已推送main；BFF当前policy `cd1c260…`，Root `83022b65…`已发布来源gitlink/库存 | 三文档独立复审0 P1/P2；W1C-2A同源GET窄relay由Web唯一writer进行中，Auth.js Code+S256/CSRF/Product Session/全代理Bearer/旧IAM直连删除仍未开始；状态：设计门已验收，源码进行中 |
| W1C-2A / P0 | Web固定policy来源与同源只读`/iam`入口 / `w1c_web_get_relay_owner`唯一Web源码writer，Root审查与Git负责人 | Web `38683a5…` main基线；BFF新`cd1c260…` policy v1 digest `457909cd…`、IAM `b838853…`；先于Web Auth.js与mutation | 首轮Web全门通过但独立审查1P1/3P2待修：固定可信Web origin/Host、raw Location编码路径、真实Next HTTP语义及文档当前/目标分界。已授权同writer扩`.env.example`与聚焦E2E/相邻INDEX，仍不改旧Auth/Product代理。GET窄白名单与POST/userinfo/end-session拒绝保持；状态：审查修复中 |
