...
| W1C-1 / P0 | BFF受控`/iam`原生协议relay / `w1c_bff_relay_owner`实现、来源重pin由Root集成 | BFF `1fae01e3…`、IAM `c9a27721…`、Web文档 `38683a5…`已推送main；Root `a5ca0afc…`锁定全部gitlink/库存 | 精确path/method服务例外、原生HTTP/cookie/redirect保持；BFF Node22标准248/248、contract25/25，新pin IAM7/Capability8/Scheduler11/System真HTTP均通过；完整OAuth退出401未过W1C-1H，状态：relay基础已验收，首次OAuth未闭环 |
...
| W1C-1H / P0 | Root真实IAM→BFF relay首次OAuth正向组合 / `w1c_oidc_smoke_writer`脚本交付待审；Root审查 | IAM `c9a27721…`、BFF `1fae01e3…`及Root gitlink已发布；两个Root脚本未提交 | 真HTTP首次未预同意Code+S256、tenant/consent、token/userinfo/get-session均200，带hint logout 401 invalid_token、无hint 400 invalid_request；自有资源清理0，完整门FAIL，不假PASS；状态：待IAM fixture 1I与Root复验 |
...
| W1C-2A / P0 | Web固定policy来源与同源只读`/iam`入口 / `w1c_web_get_relay_owner`唯一Web源码writer，Root审查与Git负责人 | Web `38683a5…` main基线；BFF `1fae01e3…` policy v1 digest `cbb62a48…`、IAM `c9a27721…`；先于Web Auth.js与mutation | 仅固定snapshot/provenance、精确安全GET路由、原生status/header/多Set-Cookie、cookie过滤、限额/取消及负例；所有browser POST、end-session、userinfo Bearer和未知路由fail closed，不修改旧Product代理/会话或IAM/BFF。Root在Web唯一writer停写后重跑Web门并提交；状态：进行中 |
