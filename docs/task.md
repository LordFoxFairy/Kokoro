...
| W1C-DB-BFF / P1 | BFF单应用库owner-schema首个代码切片 / `w1c_bff_logout_metadata_reader`续任BFF唯一writer；Root审查、Git唯一负责人 | BFF main `cd1c2600ea2a6e0716b07628822a49653964675a` clean，Root `85e06c53…`；限BFF现有`database/schema.sql`、`scripts/apply-schema.mjs`、`src/infrastructure/postgres/client.ts`、URL配置/对应测试及本仓三设计文档，具体文件集先经第8节放置表冻结 | 单库同账号、BFF独立schema的fresh install只检查本schema；旧public误接、其他owner表、重复安装/漂移负例；原owner测试和临时资源清理。禁止触碰Web、Root、IAM及其他owner，不做多角色/生产部署。状态：并行唯一写入进行中，文档门先行 |
