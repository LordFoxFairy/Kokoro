...
| W1C-2F-S1 / P0 | Product Session store、callback与logout/refresh基础代码 / Web `w1c_web_cutover_audit`唯一源码writer，Root审查/Git | Web main `5da730426faaca54a9f0003fa1e7fd99f4db6f00` clean；固定BFF `eb7ded2…`/IAM `65b0fd9…`，依赖2F-D/P；仅现有RP route/provider、`src/lib/server/{product-session,product-session-store,oidc-token}.ts`及对应server/system测试、必要Web CURRENT/三文档当前态 | TDD先反转受控503；有效callback创建在线会话，公开session无token；Redis双CAS/单次refresh/active-current logout/pending tombstone、竞争/重放/故障测试；Node22全门与真实三仓HTTP。普通代理Bearer/旧路由删除为后续S2，不把S1称完整W1C。状态：进行中 |
| W1C-Team-D / P0 | IAM当前tenant roster/invitation/role窄读三文档门 / IAM `w1c_session_semantics_audit`唯一文档writer，Root审查/Git | IAM main `65b0fd969989d4044fae640a8414d9c2dcf41c3b` clean；只改TECHNICAL_DESIGN/API_CONTRACT/DATA_MODEL，Web源码与IAM文档跨仓可独立并行 | 三个user-only read scope、持久权限、最小字段/分页/索引/issuer pending边界与当前/目标态一致；Root审查后才准runtime/机器契约写入。状态：进行中 |
