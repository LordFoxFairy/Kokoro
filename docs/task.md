...
| W1B-3C / P0 | Root旧Capability/Scheduler smoke准入改造 / `w1b3_legacy_smoke_writer` / gpt-6-sol / 唯一脚本writer；Root审查、Git与台账负责人 | Root main `1d1c4df…`，9个获授权脚本/测试，新增共享IAM wire stub，未改子仓/System；Root精确提交待形成 | Root聚焦**110/110**、全scripts **524/524**，真实Capability **8/8**、Scheduler **11/11**及自有资源清理均通过；独立审查2项P2已修复复核无新阻断。stub只供旧owner回归；状态：待集成验证 |
| W1B-3D / P0 | Root System跨仓smoke新准入与路径修复 / 待派工；Root审查与Git负责人 | 依赖W1B-3C共享IAM stub先提交；仅`scripts/e2e/run_system_owner_smoke.py`、`scripts/tests/test_system_owner_smoke.py`，不碰System/BFF/Agent子仓 | `ROOT/apps/owner`路径、BFF models Bearer A/B、server-only manifest真实tenant、原System/Agent读链及跨tenant负例；真实System↔BFF↔Agent HTTP、独占资源与当前SHA门；状态：待派工 |
