...
| W1C-S2-HTTP / P0 | 真实 HTTPS 浏览器 Product Session → 普通 Web Chat 代理 → BFF listSessions / Root唯一脚本writer、独立审查 | Root `e682bab689db0aaca815f90af6c0e594e03e6b76`、IAM `c0f6068`、BFF `1917f90`、Web `670f0ff`；只改现有 Product Session smoke及其单测和Root台账，不改业务子仓，与Storage schema writer独立 | 匿名/旧代/退出后均401且零BFF Chat请求；在线会话使用 cookie，经Web `/api/session/sessions` 访问BFF `/v1/sessions` 返回200平面列表、no-store、request ID，浏览器自报 Authorization 不覆盖在线 Bearer；TDD聚焦单测、固定SHA真HTTPS及自有资源0。状态：待集成验证；首轮真实HTTPS在JSON成功响应缺private处RED，Web已定点修复发布，Root脚本聚焦11通过/41 subtests，固定新SHA真实复验待Root发布gitlink后运行。 |
| W1C-S2-Web-Cache / P0 | Chat 同源代理成功响应保持 `private, no-store` / Root定点小修（子代理并发额度占满）；独立脚本审查 | Web `670f0ff77ec71aaa03c15097587cc1071f85cfd9` main/clean；只改现有 `src/app/api/session/[...path]/route.ts`、`tests/app/session-proxy.test.ts`及必要相邻测试，不改Root/BFF/IAM/Storage，Root独占Git index | Root固定SHA真HTTPS S2 首次 RED：匿名及在线身份链已走到普通Chat代理，成功响应仅 `no-store`，不满足受保护响应 `private, no-store`；TDD正负用例后 Node22 Web focused/full gate，Root再跑真实HTTPS。状态：Web源码已验收并发布 main `8a8347b99ab20d44a024fc2f437c4bffd246601f`；focused 7/7、Node22 check contract52/architecture32/Vitest1384/lint/typecheck/build、Playwright9通过/1既有skip。Root最终真HTTPS待发布gitlink后复验。 |
