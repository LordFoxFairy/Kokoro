...
| W1C-2D / P0 | Root真实 HTTPS Web→BFF→IAM RP-only 组合 / `w1c_root_smoke_hardening_sol`唯一Root脚本writer；`w1c_iam_fixture_identity_sol`唯一IAM测试fixture writer；Root审查/Git | Web `5299f29…`已发布且独立Node22全门1345/1345、Playwright6/6，IAM `6bc9b19…`/BFF `a4dbc33…`当前不变；Root两新增脚本未提交，Root独占docs/index/commit | 真实HTTPS链已穿过sign-in、tenant和consent GET，consent POST原报503，已定位IAM成功URL固定`code/state/iss`而Web两处仅准双键；Web严格三键修复经独立审查0 P0/P1/P2，需复跑真链。Root脚本二轮审查P1日志扫描漏真实token/Basic、IAM ready前资源身份盲窗；现并行收敛Root脚本与IAM测试fixture可推导身份，不将RED或fixture当三仓已通。状态：进行中 |
