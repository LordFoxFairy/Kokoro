...
| W1C-Team-R1 / P0 | IAM当前tenant成员/邀请/角色三个窄读GET的owner runtime、canonical索引、OpenAPI/SDK与真实验收 / `iam_team_read_owner`唯一IAM writer；Root审查/Git | 基线IAM `f240bd7d5f542bb152c7eb929074c96b6c290ea8`、`main`、clean；三文档 `/Users/nako/WebstormProjects/github/thefoxfairy/Kokoro/apps/kokoro-iam/docs/{TECHNICAL_DESIGN,API_CONTRACT,DATA_MODEL}.md` 的W1C-Team-D设计门已对齐；机器契约/SDK与运行端点尚无。只写IAM `src/modules/{auth/oauth,auth/better-auth,auth/internal-access,tenancy/organization}`相邻源及测试、`src/modules/tenancy/tenancy.module.ts`仅DI、`src/http`既有no-store路由matcher及测试、既有`sdk/src/` facade/types、`scripts/generate-contracts.ts`仅三GET生成、`prisma/schema.prisma`、owner生成contract/SDK与IAM CURRENT/ACCEPTANCE；排除Root/Web/BFF、其他IAM业务 | 三个user-only scope不进共享`TENANT_SCOPES`或machine；Guard显式list读分支，同一RepeatableRead快照验证当前membership/role/permission/tenant再查询；固定权限矩阵+动态角色、HKDF cursor、keyset分页、三索引、损坏依赖503、跨tenant/机器/失效401/403，真实Code+PKCE/refresh与PG/Redis测试，fresh schema/零FK/EXPLAIN，生成OpenAPI/SDK+consumer；`pnpm verify`、integration、consumer、build。Agent交付文件清单不动Git index/commit，Root在冻结树独立重跑并提交、推送、提升gitlink。状态：进行中；BFF/Web只读消费审查已结束，Web Product Bearer切片独立并行 |
