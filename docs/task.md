...
| W1C-2 / P0 | Web Auth.js RP、同源adapter与Product Session / `w1c_web_design_owner`三文档writer；源码唯一writer分片推进 | Web三文档门`62da3f85…`，2A源码`85b4bad…`及Root gitlink/库存`6f7b31c…`已推送main；BFF当前policy `cd1c260…` | 三文档门与2A同源GET窄relay已验收；Auth.js Code+S256/CSRF/Product Session/全代理Bearer/旧IAM直连删除仍未开始；状态：Web源码部分完成，2B待实施 |
...
| W1C-2B-1 / P0 | Web交互POST/CSRF与最小真实续接 / `w1c_web_post_foundation_owner`唯一writer；Root审查/Git | Web `85b4bad25769efcca8f417e0232fdaa6c485bf01`→`d619f2c06951cb2decdb1eeac48547e3bcf40361` 已推送main且clean；27文件，含Redis server-only依赖/lock、env/CI与现有原生Location策略定点扩展 | 独立复审原1 P1/2 P2已修复0 P1/P2，P3 302回归亦补；Root Node22 `pnpm check` contract52/architecture32/标准1297与lint/typecheck/build、Playwright6/6，Redis9 0→0。仅sign-in交互，Auth.js/Product Session/旧IAM直连仍待后续；Root gitlink/组合未集成。状态：Web owner代码已验收、Root集成待办 |
| W1C-2B-2 / P0 | Web Tenant选择与Consent两页安全续接 / `w1c_web_post_foundation_owner`续任Web唯一writer；Root审查/Git | Web main `d619f2c06951cb2decdb1eeac48547e3bcf40361` clean；`/auth/{select-tenant,consent}`仅精确GET同源跳转，真正GET/POST页面放Web-owned静态`/iam/interactions/{select-tenant,consent}`，复用server-only relay/Redis CSRF；仅Web相关tests/文档，不改IAM/BFF/Root | 独立审查首轮1 P1：issuer cookie固定`Path=/iam`，原`/auth/*`页面收不到；按上述路由重构并以真实Next静态优先+遵守Path的CookieJar测试sign-in→tenant→consent，直`/iam` owner mutation仍拒绝。Tenant owner list/POST重核；consent scope仅未验签预览、IAM POST最终验签；RP callback尚未安装时受控503。Node22全门/Root复验后才提交；状态：审查修复中，未验收 |
...
| W1C-DB-BFF / P1 | BFF单应用库owner-schema首个代码切片 / `w1c_bff_logout_metadata_reader`唯一writer；Root审查/Git | BFF `cd1c2600…`→`06a478403c92eeede0c54ed1f53022f0ff60d79e` 已推送main且clean；29文件，canonical SQL/OpenAPI/package/lock未变 | 独立复审0 P1/P2；Root Node22 format/check 253/253、真实schema 6/6、独占PG+Redis integration 37/37通过，自有DB已删、Redis8 0→0。Root gitlink/库存尚待集成；全量catalog drift另片，不宣称所有owner单库完成；状态：BFF owner代码已验收、Root集成待办 |
| W1C-DB-BFF-ROOT / P1 | Root现有跨仓smoke消费BFF新schema / `w1c_oidc_runner_rereview`唯一writer；Root主控审查/Git | Root main `7ec2301af044a9da759968be8b6dd260a3d53df0`；五runner、窄helper及测试共10文件候选，未操作Git/index、owner源码/库存/控制文档 | BFF URL显式`schema=kokoro_bff`，删旧public PGOPTIONS，直接SQL指向owner schema，固定BFF SHA→`06a4784…`。Root独立聚焦151 passed/56 subtests、Ruff/compile/diff-check通过；真实组合须在gitlink集成后运行。状态：Root脚本代码待集成验收 |
