...
| W1C-2B-1 / P0 | Web交互POST/CSRF与三页最小真实续接 / `w1c_web_post_foundation_owner`唯一Web writer；Root审查、Git唯一负责人 | Web main `85b4bad25769efcca8f417e0232fdaa6c485bf01`；现有IAM relay/新交互页/CSRF helper、对应tests、必要文档；批准Redis server-only依赖与lock、五份env样例、三份CI workflow及架构门用于真实一次性CSRF，不扩入Auth.js/Product Session/旧登录/普通v1代理 | TDD覆盖Origin、一次性CSRF、重放、错path、body/cookie上限及零BFF socket；独立审查1 P1/2 P2/1 P3需修真实continue 200 JSON导航、非200中间响应泄漏、Redis测试清理边界，唯一writer修复中。Node22本仓全门与真实Next HTTP、Root复跑后才提交；状态：审查修复中，未验收 |
...
| W1C-DB-IAM / P1 | IAM owner-schema适配前置只读设计审查 / `w1b3_iam_fixture_reader`；Root裁决 | IAM main `b838853a81ff34bd0f7a079ccc75ba6abd61d1ec` clean；只读postgres-url/Prisma/fresh installer/runtime/fixture/CI与三设计文档，不操作Git/共享资源 | 输出精确public与整库假设、最小代码文件集、同库共存/回滚/缺表ready验收方案；当前未授权IAM源码写入。状态：只读审查进行中 |
