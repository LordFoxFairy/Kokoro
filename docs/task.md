...
| W1C-2 / P0 | Web Auth.js RP、同源adapter与Product Session / `w1c_web_design_owner`三文档writer；源码唯一writer分片推进 | Web三文档门`62da3f85…`，2A源码`85b4bad…`已推送main；BFF当前policy `cd1c260…`，Root本次候选提升Web gitlink/库存 | 三文档门与2A同源GET窄relay已验收；Auth.js Code+S256/CSRF/Product Session/全代理Bearer/旧IAM直连删除仍未开始；状态：Web源码部分完成，2B待实施 |
| W1C-2A / P0 | Web固定policy来源与同源只读`/iam`入口 / `w1c_web_get_relay_owner`唯一Web源码writer，Root审查与Git负责人 | Web `38683a5…`→`85b4bad25769efcca8f417e0232fdaa6c485bf01`已推送main，BFF `cd1c260…` policy digest `457909cd…`字节相同；Root本次候选提升gitlink/库存 | 两轮审查发现的P1/P2全修，增量独立复审0 P1/P2；Root独立Node22 `pnpm check`（1259 tests）与Playwright6/6、Root 563脚本、policy/checkpoint/topology通过，真实Next HTTP含chunked拒绝/零BFF及隔离目录清理。GET窄子集已发布；Auth.js/POST/userinfo/end-session仍未接线，浏览器登录不成立；状态：Web owner已验收、Root集成候选已验 |
