...
| W1C-1H / P0 | Root真实IAM→BFF relay首次OAuth正向组合 / 待派Root脚本writer，Root审查 | IAM `f0bb18e6…` test-owned host、BFF `804a5832…`固定policy及Root `d7ad9331…`gitlink已发布clean | 真实HTTP经过BFF `/iam`证明首次未预同意Code+S256、签名交互续接、token/userinfo/logout与受控cookie/header，隔离资源清理；只新增Root e2e runner与聚焦测试，不修改owner源码、IAM数据库事实或Web页面；状态：待派工 |
