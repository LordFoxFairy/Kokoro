...
- 当前关键路径：**W1C Product Session → 普通BFF代理Bearer与旧直连删除 → Team owner-first窄契约 → W1D Product generated consumer/AG-UI → Storage**。真实HTTPS Web→BFF→IAM RP-only 已在固定三仓来源上通过14/14，但终态仍是受控503、没有Product Session；不能把它视为登录或完整聊天闭环。具体状态与唯一写入负责人见Wave1C执行卡。
...
| W1C-2F-S1-HTTP / P0 | Root真实HTTPS Product Session组合验收 / `product_smoke_plan`首版脚本writer、Root续写/审查/Git | 新Root runner及测试两文件已随 `594358ba…` 提交；首次固定Web `1ba0498…`在Product Cookie Secure处RED。`0c829a0…`固定Web `0e0ec3a…`后二次真HTTPS到IAM issuer confirmation GET仍得`400 invalid_request`：Node22 BFF内部fetch自动`sec-fetch-mode:cors`，均清理自有资源。BFF `ddb462e…`、Web `c3d81bf…`已各自推送main，Root新gitlink/库存待提交 | Root runner已加旧generation signout真负例与受控IAM错误码诊断，聚焦pytest9/30 subtests、Ruff通过；Root scripts全量此前596/106。真callback→cookie→refresh→旧代拒绝→当前logout/revoke→IAM确认/失效与资源零残留须在新三仓pin实跑；S2普通Product代理Bearer不在此片。状态：两次真实RED均已定位，三仓最终复验待执行 |
| W1C-S1-HTTP-BFF / P0 | 无hint退出浏览器导航跨BFF relay / `bff_logout_navigation_fix`为BFF唯一源码writer；Root审查/Git | BFF `1d1f427…`→`ddb462e6ab3a7270a3dab248ba7ee887b0ec9ba2` 5文件已提交推送main且clean；Root gitlink/库存待跟进 | Node22 Undici即使显式设置navigate也强制发送cors（TDD红）；只对已准入`/oauth2/end-session` GET走固定IAM origin的有界原生HTTP，内部合成navigate，其他路由仍fetch；独立审查0 P0/P1/P2。Root Node22 `pnpm check` 255 pass/1既有skip、lint/typecheck/contract/build通过；真HTTPS新pin待复验。状态：BFF owner代码已验，Root组合待验 |
| W1C-S1-STALE / P0 | 旧generation Cookie不得退出当前会话 / `web_stale_signout_fix`为Web唯一源码writer；Root审查/Git | Web `0e0ec3a…`→`c3d81bf16fc62cef59ab33f6cc61a6677a38383c` 10文件已提交推送main且clean；含BFF `ddb462e…`来源机械pin；Root gitlink/库存待跟进 | Redis Lua原子CAS失配不删最新session、不revoke、不返回issuer退出引导；独立初审P2发现旧响应清同名cookie会删除并发新代cookie，已修为stale响应无Set-Cookie，终审0 P0/P1/P2；单测+真实Next HTTP覆盖旧cookie signout后新cookie仍有效。Root Node22 contract52/architecture32、Vitest1371、lint/typecheck/build、Playwright6均PASS；Root真实HTTPS runner已补stale负例，待新pin执行。状态：Web owner代码已验，Root组合待验 |
