...
| W1C-DB-IAM-C / P1 | IAM单库owner-schema代码实现 / `w1b3_iam_fixture_reader`续任IAM唯一源码writer；Root审查/Git | IAM main `d415ddbe565ec6c09e432945624e72c4b86d7a78` clean；仅Prisma canonical、URL/adapter/runtime/readiness、fresh installer/catalog、IAM本仓fixture/tests、env/release workflow及README/RUNBOOK/CURRENT必要修正；不碰Root/BFF/Web或API wire | TDD四项真实PG coexist/nonempty/rollback/missing-table及17表owner、raw SQL命名、runtime URL拒绝/ready；Node24 Prisma validate/generate、format/lint/typecheck/标准/聚焦integration/build。测试临时DB/Redis prefix独占清理，无新role/生产运维扩大；状态：IAM唯一writer进行中 |
