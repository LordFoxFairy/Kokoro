...
| W1C-2F-S1 / P0 | Product Session store、callback与logout/refresh基础代码 / Web `w1c_web_cutover_audit`唯一源码writer，Root审查/Git | Web `5da7304…`→`1ba0498511447b9aafde892adafaae4de7f61ed6` 精确17文件已提交推送main且clean；BFF/IAM新来源已消费；Root gitlink/库存待本次提交 | callback创建加密HttpOnly会话；Redis双CAS、单次refresh、active-current revoke/pending tombstone；退出只返回待浏览器确认的issuer URL，实际confirm才删IAM session。独立终审0 P0/P1/P2；Root Node22在宿主睡眠复现误超时后用caffeinate重跑contract52/architecture32、Vitest1366、lint/typecheck/build、Playwright6均PASS。真实三仓HTTP仍待W1C-2F-S1-HTTP；普通代理Bearer/旧路由删除为S2。状态：Web owner代码已验，Root组合待验 |
...
| W1C-Team-P / P0 | IAM文档新来源→BFF relay机械pin / BFF唯一writer、Root审查/Git | BFF `eb7ded2…`→`1d1f42775e0fa4464de6b08ee9d2b9cd82911a71` 已提交推送main且clean；Web `1ba0498…` 已消费；policy字节digest `bbd86696e1b36a82c1ebd35262dba3950a35d56d7d63856df217f397d8b48819` | BFF仅四处来源pin；Node22 policy聚焦4/4、contract check通过；Web新提交含消费pin与完整Node22门，Root gitlink/库存尚待最终集成。状态：BFF/Web owner已验，Root待集成 |
| W1C-2F-S1-HTTP / P0 | Root真实HTTPS Product Session组合验收 / `product_smoke_plan`唯一Root脚本writer、Root审查/Git | 新Root runner及测试两文件已冻结未提交，复用旧RP-only fixture的来源/自有资源/HTTPS入口，不沿用受控503断言；固定Web `1ba0498…`/BFF `1d1f427…`/IAM `f240bd7…` 待Root gitlink发布 | 独立终审初1 P1/4 P2均定点修，终审0 P0/P1/P2；Root聚焦pytest8/28 subtests、Ruff通过。真callback→cookie→refresh→logout/revoke→IAM确认/失效与资源零残留须在Root pin发布后实跑；S2普通Product代理Bearer不在此片。状态：脚本已审未发布，真实三仓未运行 |
