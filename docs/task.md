...
| W1C-Team-R2 / P0 | BFF IAM Team 三读的公开Product投影与generated consumer / BFF单仓唯一writer；Root审查/Git | IAM owner `68aa0da` 已提交推送main，OpenAPI/SDK 0.3.0、真实PG/Redis与clean consumer已验；BFF基线`ddb462e6ab3a7270a3dab248ba7ee887b0ec9ba2`、main/clean。先更新BFF TECHNICAL_DESIGN/API_CONTRACT/DATA_MODEL三设计门与IAM dependency/vendor provenance；范围限BFF Product OpenAPI、generated IAM client、IAM Team窄adapter、user admission request-scoped Bearer、route/测试/CURRENT，Root持Git。 | 公开GET `/v1/team/{members,invitations,roles}`从已验证User admission取得tenant/subject/token，同一个Bearer调用IAM三GET；不接收自报tenant/subject，不用machine，不读IAM DB，不存储Team事实。严格limit/cursor、错误/Retry-After/no-store/请求ID、取消和超限；owner schema不符502。Node22 contract/architecture/check/build与真实IAM HTTP负例；不伪造本人pending invite、写操作和team switch。发布后Web才消费。本片状态：进行中；主控因本线程子代理额度已满接手唯一BFF writer，先发布IAM 0.3.0 immutable vendor/四operation generated consumer `e79dda6111427a477d83cf1ac7783b106af6fb9c`；Node22 `pnpm check` 255通过/1既有skip、format/check、generator双次drift与聚焦contract14通过。公开Product三GET、真实IAM HTTP、relay/Web re-pin尚待实现，不算R2完成 |
