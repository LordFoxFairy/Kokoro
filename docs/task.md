...
| W1C-2B-1 / P0 | Web交互POST/CSRF与三页最小真实续接 / `w1c_web_post_foundation_owner`唯一Web writer；Root审查、Git唯一负责人 | Web main `85b4bad25769efcca8f417e0232fdaa6c485bf01` clean，Root `85e06c533c4141cea1aba28cca6b18c5e0ae9131`；仅Web现有IAM relay/新交互页/CSRF helper/对应tests及必要当前态文档，文件表由writer按第8节先冻结 | TDD：Origin、一次性CSRF、重放、错path、body/cookie上限本地拒绝且零BFF socket；合法IAM签名续接由owner验证，真实Next HTTP；不得改Auth.js、Product Session、旧登录/普通v1代理、BFF/Root/数据库或Git。Node22本仓聚焦/contract/architecture/lint/typecheck/build，Root复跑；状态：并行唯一写入进行中 |
...
| W1C-DB-BFF / P1 | BFF单应用库owner-schema首个代码切片 / `w1c_bff_logout_metadata_reader`续任BFF唯一writer；Root审查、Git唯一负责人 | BFF main `cd1c2600ea2a6e0716b07628822a49653964675a` clean，Root `85e06c53…`；限BFF现有`database/schema.sql`、`scripts/apply-schema.mjs`、`src/infrastructure/postgres/client.ts`、URL配置/对应测试及本仓三设计文档，具体文件集先经第8节放置表冻结 | 单库同账号、BFF独立schema的fresh install只检查本schema；旧public误接、其他owner表共存、重复安装/回滚及runtime `current_schema()`负例；原owner测试和临时资源清理。全量catalog drift不冒称本片已实现。禁止触碰Web、Root、IAM及其他owner，不做多角色/生产部署。状态：并行唯一写入进行中，文档门先行 |
