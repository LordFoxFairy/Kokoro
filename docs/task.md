...
| W1B-3B / P0 | Root真实IAM↔BFF组合 / `w1b3_root_iam_smoke_writer` / gpt-6-sol / 仅两脚本写入；Root审查与Git唯一负责人 | Root main 77c9c5db；依赖IAM `b2ad9dd…`协议已冻结；仅`/Users/nako/WebstormProjects/github/thefoxfairy/Kokoro/scripts/e2e/run_bff_iam_session_smoke.py`、`scripts/tests/test_bff_iam_session_smoke.py`，Root台账由主控独写，BFF/IAM代码不改 | 当前源码构建后真实HTTP 7/7、聚焦8/8、Root全量499通过；独立审查P1/P2已修复。待Root gitlink提交后无过渡参数复验；状态：待集成验证 |
