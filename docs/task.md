...
| W1C-1F / P0 | IAM test-owned Web OIDC首次流fixture / 待派唯一IAM writer；Root审查 | W1C-1 BFF代码审查后；IAM b2ad9dd…当前fixture可复用DB/Redis/Nest，旧userClient已预consent不适合证明首次流 | 可配置Web origin的真实IAM host、第二个未预consent RP client与测试专用email/password；owner自测真实sign-in→authorize首次重定向及资源清理，完整Code+S256/tenant/consent/token/userinfo/logout由Root组合验证；仅测试所有权，不改生产API/schema；状态：待派工 |
| W1C-1G / P0 | Root从gitlink commit blob核验BFF relay↔IAM发布证据 / 待派Root治理writer；Root集成 | BFF policy发布后；IAM b2ad9dd…、BFF待发布SHA | 固定IAM allowlist/snapshot与BFF policy的commit/digest、route/method子集/disabled负例；不读取脏子仓工作树，篡改SHA/blob/route负例红；不复制IAM私有源码到BFF；状态：待派工 |
