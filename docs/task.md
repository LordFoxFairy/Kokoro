...
- 当前关键路径：**W1C 普通BFF代理Bearer真实组合与旧直连删除 / Web Team Product消费及其缺失的IAM owner契约 → W1D Product generated consumer/AG-UI → Storage与聊天执行链**。固定Web `670f0ff…`/BFF `1917f90…`/IAM `c0f6068…` 的真实HTTPS S1登录、刷新、退出链已通过且自有资源0；IAM→BFF Team三读真实HTTP 19 case已通过，但Web Team消费、旧直连删除及完整聊天仍未闭环。具体状态与唯一写入负责人见Wave1C执行卡。
...
| W2-DB-Storage | P0 | Storage 从 `public`/整库空白安装改为单应用库的 `kokoro_storage` owner schema | Storage / 唯一子代理writer；Root审查、提交与组合验收 | 已发布的 Storage F1，不依赖 Team UI；不激活 Storage consumer | 同库已有其他 owner schema 时只创建/检查自己的 schema；空 owner schema 安装、重入拒绝、catalog/drift 与实际 Prisma 读写均通过；不碰其他 schema、无需多角色；状态：进行中，基线 Storage `f80917e98a1cd1fe196ce10b0aba6f8f67fdf205` main/clean |
...
| W1D-Chat-P0 / P0 | 首条消息建立会话并让Agent结果成为持久可重载的assistant消息 / Agent契约owner→BFF实现→Web消费，逐仓唯一writer | 固定审计基线BFF`1917f90`、Agent`741c…`、Web`7c0ab98`；只读跨仓审计发现BFF没有生产`bff_conversation` INSERT且AG-UI投影未回写`bff_message`；本卡不授权越界同时改三仓。先由Agent将现有LaunchReceipt/ReplayPage绑定operation typed envelope，再由BFF依次完成首消息同事务隐式建会话、assistant source-event幂等回写，最后Web generated消费。 | 新会话Web首发202；BFF同事务Conversation+user+pending assistant+outbox，项目/tenant/subject校验；真实Agent worker产出delta/completed/failed；BFF AG-UI与message同事务去重，snapshot同快照watermark，reload仍见completed非空assistant；跨tenant/重复事件/崩溃重放负例，固定SHA真Web→BFF→Agent HTTP/worker/PG/Redis smoke。状态：待Agent owner契约文档门与切片派工；纯文本fixture不等于真实System/LiteLLM模型路由。 |
