...
| W1C-2D / P0 | Root真实 HTTPS Web→BFF→IAM RP-only 组合 / Root下一脚本writer待派；Root审查/Git | 前置Web `c71aa3f…`、BFF `a4dbc33…`、IAM `6bc9b19…`；Root复用现有隔离 IAM/BFF/PG/Redis fixture，新增测试自有 HTTPS Web 启动；不改 owner 生产代码/schema/contract | 浏览器同源 CSRF→signin→tenant→consent→callback，真实IAM EdDSA/JWKS/token/userinfo、最终503无新session；重放、错误/越权负例与自有资源清理。先完成Root放置表/脚本精确范围和真实命令，再授权writer。状态：待派工，现有Web+BFF stub不冒充完成 |
