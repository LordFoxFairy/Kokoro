...
- 当前关键路径：**W1C BFF/IAM首次OAuth正向组合 → Web OIDC/同源接线 → W1D Web Product generated consumer/AG-UI → Storage owner与消费边界**；BFF relay 来源门在新pin已验证，原IAM准入、Capability/Scheduler/System旧真实回归绑定前一`1fae01e…` release，新`cd1c260…`待重跑；这些均不代表Web或完整聊天执行链已闭环。具体状态与唯一写入负责人见本文Wave1C执行卡。
...
| W1C-1 / P0 | BFF受控`/iam`原生协议relay / `w1c_bff_relay_owner`实现、来源重pin由Root集成 | BFF `cd1c2600…`、IAM `b838853a…`已推送main；Root新gitlink/库存候选待发布 | 精确path/method服务例外、原生HTTP/cookie/redirect保持；BFF Node22标准248/248、contract25/25；`1fae01e…`真IAM7/Capability8/Scheduler11/System已过，新pin及首次OAuth完整链待重跑，状态：待新pin集成验证 |
...
| W1C-1H / P0 | Root真实IAM→BFF relay首次OAuth正向组合 / 原脚本writer交付、`w1c_oidc_runner_fix`唯一Root脚本修复writer；Root审查 | IAM `b838853…`、BFF `cd1c260…`已发布，Root新gitlink待发布；两Root脚本未提交 | 旧pin真HTTP首次Code+S256至userinfo 200，logout 401；IAM fixture已修，脚本独立审查2P1/2P2正按令牌绑定、凭据日志、logout state/cookie、revoke实际失效修复；新pin真实完整门未跑，状态：进行中 |
| W1C-1I / P0 | IAM test-owned OIDC host精确public JWKS回环 / `w1c_iam_flow_owner`唯一IAM writer；Root审查/Git | IAM `b838853a81ff34bd0f7a079ccc75ba6abd61d1ec`已提交推送main，BFF机械policy来源`cd1c260…`已发布 | 仅IAM两测试文件，精确issuer `/jwks`→fixture loopback；Root独立Node24真实登录→带hint退出聚焦5/5、format/lint/typecheck、资源0；独立审查0 P1/P2，非阻断P3 restore直接断言缺口；生产代码/contract/schema不变。状态：IAM owner切片已验收，Root组合待复验 |
...
| W1C-2 / P0 | Web Auth.js RP、同源adapter与Product Session / `w1c_web_design_owner`三文档writer；源码唯一writer分片推进 | Web三文档 `62da3f85…`与最终来源pin `38683a5…`已推送main；BFF `1fae01e3…` policy digest固定，Root `a5ca0afc…`已发布gitlink/库存 | 三文档独立复审0 P1/P2；W1C-2A同源GET窄relay由Web唯一writer进行中，Auth.js Code+S256/CSRF/Product Session/全代理Bearer/旧IAM直连删除仍未开始；状态：设计门已验收，源码进行中 |
| W1C-2A / P0 | Web固定policy来源与同源只读`/iam`入口 / `w1c_web_get_relay_owner`唯一Web源码writer，Root审查与Git负责人 | Web `38683a5…` main基线；BFF新`cd1c260…` policy v1 digest `457909cd…`、IAM `b838853…`；先于Web Auth.js与mutation | 固定snapshot/provenance、精确安全GET路由、原生status/header/多Set-Cookie、cookie过滤、限额/取消及负例；browser POST、end-session、userinfo Bearer和未知路由fail closed。授权同一writer对Web既有三份设计文档做SHA/digest机械re-pin，不改设计语义。Root停写后验收提交；状态：进行中 |
