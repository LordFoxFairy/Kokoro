...
| W1C-2F-D / P0 | 会话语义冲突收敛 / Root裁决，IAM/Web各仓唯一文档writer | Web `bbd8f08…`→`ebd7703a1adfccf0f6c56e746d9735ac47d63021`三文档；IAM `606d909…`→`65b0fd969989d4044fae640a8414d9c2dcf41c3b`四文档/ADR，均推送main且clean；Root保留Git/集成 | V1双CAS/单次refresh/旧generation拒绝；Redis Web加密当前refresh，active logout取已确认当前credential，pending只tombstone不revoke旧token；竞争败者不撤销赢家。独立审查2个P2已修，Web contract52/architecture32、IAM contract check与Root tests588通过；Root gitlink/库存仍待本波集成。状态：待集成验证 |
| W1C-2F-P / P0 | 新IAM来源→BFF→Web policy provenance闭环 / BFF与Web各仓串行唯一writer，Root集成 | IAM `65b0fd9…`；BFF `2d951e1…`→`eb7ded2386efd9a10905843a7a5aedff9ac72df6`；Web `ebd7703…`→`5da730426faaca54a9f0003fa1e7fd99f4db6f00`，均推送main且clean | BFF4文件、Web7文件机械pin；policy原字节一致，SHA-256 `05e2068376ef79b6aba8eff0f170a3a2bd0a0a5b31bc6836b3de9f682ff86a10`；Node22聚焦contract/architecture通过，未改路由语义。Root inventory/gitlink/checkpoint/主仓验收待完成。状态：待集成验证 |
| W1C-2F-S / P0 | Product Session代码与真实登录 / Web唯一源码writer，Root审查/Git | 依赖2F-D Web文档门；保留已验RP-only、固定BFF/IAM来源，不重写OIDC；精确文件集与RED基线由Root在文档门后冻结 | callback建立Web在线session；Redis双CAS/加密当前refresh/tombstone、单次refresh、当前token logout、并发/失联负例；真实Web→BFF→IAM从受控503变成可用登录。状态：待派工 |
