...
| W1C-2C / P0 | Web Auth.js RP + server-only token/userinfo / Web下一唯一writer；Root审查并限定文件集 | 前置已满足：Web `14e23e6…`与Root gitlink/来源已发布；当前旧`auth.ts`仍直连IAM，最终callback受控503。只读Web设计门与IAM owner wire核对已交：候选next-auth v4.24.15/openid-client v5.7.1须按当前Next16/Node22锁定，浏览器resource/scope/redirect_uri覆盖与自定义token.request跳过state/nonce/ID token验证是硬门 | 本片仅RP transaction/callback+Web server-only Basic form token/userinfo：固定issuer/client/URI/resource，显式调用验证型client.callback，Redis一次性state，真实wire/重放/泄漏负例；Product Session未建前必须不发新session cookie且受控失败，旧路径不作为新成功。根目录放置表/精确文件集见progress；Root放行后RED→GREEN。状态：只读设计完成、代码待派工 |
