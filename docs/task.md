...
| W1C-2F-S1-HTTP / P0 | Root真实HTTPS Product Session组合验收 / `product_smoke_plan`唯一Root脚本writer、Root审查/Git | 新Root runner及测试两文件已随 `594358ba…` 提交；首次固定Web `1ba0498…`/BFF `1d1f427…`/IAM `f240bd7…` 真HTTPS在Product Cookie Secure断言处RED，自有资源已清理。Web `0e0ec3a…`与Root IAM测试issuer cookie断言定点修复后待新Root pin | 独立终审初1 P1/4 P2均定点修，最新runner断言修订再审0 P0/P1/P2；Root聚焦pytest8/28 subtests、Ruff通过。真callback→cookie→refresh→logout/revoke→IAM确认/失效与资源零残留须在新Root pin上重跑；S2普通Product代理Bearer不在此片。状态：首次真实RED已定位，修复待组合复验 |
