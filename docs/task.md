...
| W1C-2F-D / P0 | 会话语义冲突收敛与Team依赖冻结 / Root裁决，IAM/Web各仓唯一文档writer | Root main `8e8cd884…`、Web `bbd8f08…`、IAM `606d909…`、BFF `2d951e1…`，均clean；只改对应owner三文档/ADR，不启动共享资源；Root保留Git/集成 | V1选Web双CAS、每次rotation最多一次IAM refresh、旧generation拒绝；Redis仅存Web密钥加密当前refresh供按session ID登出原子take；竞争败者不撤销赢家。IAM保留30秒issuer replay事实，但删除Web必须败者恢复的旧目标指令；文档三面一致并审查。状态：进行中 |
