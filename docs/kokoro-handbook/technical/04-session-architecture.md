...
`kokoro-session` 是三仓里的会话域服务。它拥有聊天窗口、消息、run、浏览器事件、SSE 和 replay 语义。
...
- 创建 `AgentExecutionManifest` 并投递给 agent。
...
ChatMessage
  siteId / sessionId / messageId / runId / role / content / parts / attachments / status

AgentRun
  siteId / sessionId / runId / inputMessageId / assistantMessageId / status / modelRef / permissionMode / sandboxMode

SessionEvent
  siteId / sessionId / runId / eventId / event / payload / createdAt
```

Session 的聊天展示主数据是 `messages`，不是从 events 每次现折。`session_events` 用于 live/replay/audit/debug。
...
1. 校验 SiteContext 和权限。
2. 校验同 session 无 active run。
3. 写 user message。
4. 创建 run，设置 `activeRunId`。
5. 构建 `AgentExecutionManifest`。
6. XADD run request。
7. 返回 `runId`、`inputMessageId`、session 状态。
...
- 只在需要读取 SiteContext、权限、entitlement、model policy 等结构化业务上下文时通过上游服务使用。
...
如果未来开放同 session 多 active run 或并行 agent handoff，需要重新设计排序模型，不能偷偷复用 V1 规则。
...
3. session 可从 Mongo replay active run 最近事件，再 tail live bus。
4. web 按 `eventId` 去重。
