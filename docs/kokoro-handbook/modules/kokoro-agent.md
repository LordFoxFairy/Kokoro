...
`kokoro-agent` 是 Kokoro 三仓运行时的执行层。它负责按
`AgentRunInput` 执行 LangChain/LangGraph agent loop，调用模型、
skills、MCP tools、内置工具、子代理和 sandbox，并产出 raw execution
events 给 `kokoro-session`。
...
Subagent / handoff 执行。
HITL 工具审批等待和恢复。
DeepAgents backend runtime: state / local_shell / e2b / custom。
...
```text
上游：
  kokoro-session 通过 Redis run request 发送 AgentRunInput。
...
BackendPolicy
  state/local_shell/e2b/custom、网络、文件、超时、资源限制。
...
tool call 状态不单独建第二事实源；需要恢复的执行细节进入 LangGraph checkpoint。
Web 可见 tool 状态只从 kokoro-session 的 events/messages/runs projection 来。
```
...
这些配置由上游解析后进入 `AgentRunInput`，agent 不跨站查询全局配置。
...
```text
kokoro-session 发送 AgentRunInput。
...
环境变量      KOKORO_REDIS_URL
              ...
              KOKORO_AGENT_BACKEND=state|local_shell|e2b|custom
              KOKORO_SANDBOX_PROVIDER=e2b|custom
              E2B_API_KEY
多 Pod        runId lease + session lease，防重复执行。
```

本地开发可用 fake model + local_shell。生产不能把 local_shell 当隔离边界，
高风险工具必须使用 state-safe 路径或显式远程 backend。

## 测试

```text
单测：
  manifest 解析、tool registry、skill loader、MCP tool wrapper、backend policy、event adapter。
...
```text
P0  AgentRunInput 类型定稿。
    DeepAgents backend 配置模型定稿。
    Permission/HITL middleware 收敛。
    Skill loader 最小实现。
