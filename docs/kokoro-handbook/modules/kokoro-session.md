...
`kokoro-session` 是 Kokoro 三仓运行时的会话域服务。它拥有聊天窗口、
消息、run、session events、SSE、replay 和 active run admission。
...
kokoro-session 构建 AgentRunInput 并投递给 kokoro-agent。
...
V1 runtime 不使用 SQLite 策略。测试保留 memory fake，运行时持久真源走 Mongo。
...
```text
P0  ChatSession/ChatMessage/AgentRun/SessionEvent 明确建模。
    AgentRunInput builder。
    Snapshot API。
    active run admission。
    relay 改为 DB-first 后 publish live/outbox。
