...
`kokoro-web` 是 Kokoro 三仓运行时的界面层。它负责 General Chat UI、
session snapshot、SSE 消费、agent activity 展示、Skill/MCP 管理入口
和用户交互。
...
```text
SessionSnapshot
  session + messages + activeRun + eventWatermark。
...
`eventWatermark` 和 SSE `id` 都是服务端 replay 语义。Web 不把它们保存成
业务 cursor，也不参与渲染排序。
...
P0  Snapshot-first session load。
    Transport 改成 POST message + GET snapshot + EventSource。
