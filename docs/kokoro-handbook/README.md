...
本目录是 Kokoro 主仓的产品手册和技术方案总入口，覆盖产品形态、跨仓架构、模块边界、业务链路、运营规范和关键 ADR。子仓 README 说明当前实现，子仓 docs 说明局部实现细节；本手册负责把全局总设计统一起来。子仓 docs 只能补实现细节，不能替代本手册。
...
```text
kokoro-web -> kokoro-session -> Redis -> kokoro-agent -> Redis -> kokoro-session -> SSE -> kokoro-web
```

```text
kokoro-web      只做 UI、SiteContext 注入、snapshot 加载、SSE 消费、Skill/MCP 入口和本地投影。
kokoro-session  拥有聊天窗口、消息、运行状态、浏览器事件和 replay 语义。
kokoro-agent    拥有 LangChain/LangGraph 执行、Skills、MCP client、工具、子代理、HITL、checkpoint、memory、sandbox。
...
旧文档中关于 `seq`、`cursor`、`lastResumeId`、`?after=`、SQLite 默认存储、浏览器手动维护 replay cursor 的叙述属于历史实现阶段资料。本手册标准：
