# ADR-023：Billing 采用模块化单体，MySQL 保证正确性，Redis 只做协调
...
跨服务事务、契约和对账复杂度。系统同时具备 MySQL 与 Redis；若把 Redis lock 或 SETNX 当作余额和幂等的
...
MySQL 8.4/InnoDB 是 payment、fulfillment、credit grant、hold、allocation、journal、receipt 和 outbox 的最终事实源。
...
3. PostgreSQL/Kafka 作为 Billing 首发必需组件：能力可行，但与现有 MySQL+Redis 基础设施不一致，先不引入。
