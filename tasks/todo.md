...
- [x] E4 (web) DONE (web 7271578): parse + reducer-accumulate the activity families (todos replaced wholesale, toolCalls timeline, subagents, thinking; persisted state schema now requires the current shape; per-turn reset). ActivityPanel renders the CC-style todo checklist + tools + subagents + collapsible thinking. lint/typecheck/test(92)/build green.
...
- [x] #6 信任模式收敛 DONE:V1 只保留 `auto/default`；历史 `plan` 档位已从 agent/session/web 运行契约删除。Default 走 HITL 审批，Auto 不拦。
