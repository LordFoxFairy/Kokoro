...
Root-owned source：`contract/proto/kokoro/credit/v1/credit.proto`。

Credit contract 独立于 Slice-A descriptor，使用 `contract/buf.credit.yaml` 构建和校验；Slice-A 的
...
Root consumer closure：`contract/credit-v1-consumers.json`。HTTP adapter 与未来 RPC adapter 复用同一 command model；数据库 writer 只存在于 `kokoro-billing` 内部 Credit bounded context。
