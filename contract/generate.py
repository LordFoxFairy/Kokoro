#!/usr/bin/env python3
"""Deterministic code generator for the runtime event contract.

contract/events.yaml is the single source for session AGUI and web render events.
kokoro-agent/src/kokoro_agent/interfaces/envelope.py is the single source for the
agent wire event shape; contract/agent_wire.py introspects it to regenerate the
session inbound Zod mirror. Run `--check` in CI to fail on drift between
generated output and committed files.
...
def emit_web_render(spec: dict) -> str:
    ...
    env = spec["envelope"]["render"]  # [eventId, sessionId, conversationId, runId]
