...
import { commandEnvelopeDigestSource } from "../../contract/generate.mjs";
...
test("generated command digest helpers tolerate boundary-selective shared primitives under strict lint", () => {
  const source = commandEnvelopeDigestSource("model-image-effect");
  const fullyUsedSource = commandEnvelopeDigestSource("admin-command");

  assert.match(source, /^\/\/ @generated by contract\/generate\.mjs; DO NOT EDIT\.\n/u);
  assert.match(
    source,
    /\/\* eslint-disable @typescript-eslint\/no-unused-vars -- shared generated digest primitives are intentionally boundary-selective \*\//u,
  );
