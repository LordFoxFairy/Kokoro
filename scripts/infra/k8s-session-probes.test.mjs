...
test("Session browser probes separate dependency readiness from process liveness", async () => {
  ...
  assert.match(readiness, /httpGet:\s*\{\s*path:\s*\/readyz,\s*port:\s*browser\s*\}/u);
  ...
  assert.match(liveness, /httpGet:\s*\{\s*path:\s*\/healthz,\s*port:\s*browser\s*\}/u);
  ...
  assert.match(startup, /httpGet:\s*\{\s*path:\s*\/healthz,\s*port:\s*browser\s*\}/u);
