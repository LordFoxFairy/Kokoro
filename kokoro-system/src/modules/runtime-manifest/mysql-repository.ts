...
export class MysqlSystemRepository implements SystemRepository {
  ...
  public async getManifest(input: Readonly<{ context: TenantRequestContext; productId: string; locale: string }>): Promise<RuntimeManifest> {
    ...
    try {
      const bindings = await client.query<Row>(`SELECT release_id FROM system_release_binding WHERE scope_type = 'site' AND scope_id = ? AND product_id = ? AND status = 'active' ORDER BY updated_at DESC, id DESC LIMIT 1`, [input.context.siteId, input.productId]);
      const binding = bindings.rows[0];
      const releaseId = binding?.release_id === undefined ? null : String(binding.release_id);
      const records = await client.query<Row>(`SELECT module_key, scope_type, scope_id, product_id, config_key, value_json, config_version, release_id, digest FROM system_config_record WHERE status = 'active' AND deleted_at IS NULL AND (release_id = ? OR release_id IS NULL) AND ((scope_type = 'site' AND scope_id = ?) OR (scope_type = 'tenant' AND scope_id = ?) OR (scope_type = 'product' AND scope_id = ?) OR scope_type = 'global') ORDER BY module_key, config_key, id`, [releaseId, input.context.siteId, input.context.tenantId, input.productId]);
