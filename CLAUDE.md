...
> **Kokoro 项目控制优先级**：`AGENTS.md` 是本仓库的项目级执行手册，优先于本文件中的通用模板。
> `AGENTS.md`、目标子仓 `AGENTS.md` 和当前用户明确要求共同构成实际执行契约；本文件只补充没有被项目手册
> 覆盖的通用工程原则。尤其是 V1 clean-slate、无迁移、无兼容双轨、单一事实 owner、UTC、无外键和共享本地
> PostgreSQL/Redis 等项目裁决，不得被本文件后文的通用示例改写。
...
1. 系统/平台指令。
2. 用户在当前会话的明确指令。
3. 根 `AGENTS.md` 与目标子仓更具体的 `AGENTS.md`。
4. 本文件（通用 `CLAUDE.md` 补充手册）。
5. 全局 `~/.claude/CLAUDE.md`。

本文件未覆盖的主题，沿用全局默认；项目手册与本文件对同一主题给出不同规则时，以项目手册为准。Commit 作者身份、平台强制署名与 PR 流程以全局规范或运行环境为准，本文件不覆盖这些外部机制。
...
- Schema、数据修复脚本和持久化格式变化必须说明数据影响、回滚/前滚方式，并同步更新
  `INDEX.md`、运行手册或数据模型文档。
- 不写会直接破坏生产数据的脚本，除非用户明确要求并提供边界条件。
- Kokoro V1 是 clean-slate：正式仓只保留当前 canonical `database/schema.sql`（或明确的唯一 ORM
  schema），不保留 `database/migrations/`、迁移编号、迁移表、migration runner、兼容双写或旧字段 alias。
  `db:apply-schema` 只负责在空数据库安装当前 Schema；需要真实数据升级时必须另行建立经过批准的部署方案，
  不把历史迁移链偷偷恢复到仓库。
- 正式 Kokoro SQL 禁止 `FOREIGN KEY`/`REFERENCES`；跨表关系由 owner application、Repository、事务、
  锁、状态机和 reconciliation 维护。具体项目规则以根 `AGENTS.md` 为准。
